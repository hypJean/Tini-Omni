...
# Import Gemma3 model classes to ensure they are registered in AutoConfig
# This is needed for transformers to recognize 'gemma3_text' model type
try:
    # Try to import Gemma3 classes to trigger registration
    import transformers.models.auto.modeling_auto
    # Force import of gemma3 module if available
    if hasattr(transformers.models.auto.modeling_auto, 'MODEL_MAPPING'):
        # Trigger lazy import by accessing the mapping
        _ = transformers.models.auto.modeling_auto.MODEL_MAPPING
...
def setup_tokenizer(train_config, model_config, **kwargs):
    ...
    if "vallex" in model_config.llm_name.lower():
        ...
        # Try to load fast tokenizer first, fallback to slow tokenizer if it fails
        trust_remote_code = "gemma" in model_config.llm_name.lower() and "3" in model_config.llm_name
        try:
...
def setup_encoder(train_config, model_config, **kwargs):
    ...
    if len(encoder_list) == 1:
        encoder_name = encoder_list[0].lower()
...
def setup_llm(train_config, model_config, **kwargs):
    ...
    # For Gemma-3 models, ensure the model type is registered before loading
    # This pre-loads the config with trust_remote_code to trigger model type registration
    if "gemma" in model_config.llm_name.lower() and "3" in model_config.llm_name:
        try:
            # Pre-load config with trust_remote_code to ensure model type is registered
            _ = AutoConfig.from_pretrained(
                model_config.llm_path,
                trust_remote_code=True
            )
        except Exception as e:
            logger.warning(f"Failed to pre-register Gemma3 model type: {e}. Will try again during model loading.")
    if (train_config.enable_fsdp or train_config.enable_ddp) and train_config.low_cpu_fsdp:
        ...
        if rank == 0:
            ...
            llama_config = AutoConfig.from_pretrained(
                model_config.llm_path,
                trust_remote_code=True if "gemma" in model_config.llm_name.lower() and "3" in model_config.llm_name else False
            )
    ...
    if getattr(train_config, "quantization", False):
        try:
            ...
            logger.warning(f"Quantization requested but peft/prepare_model_for_kbit_training not available: {e}")

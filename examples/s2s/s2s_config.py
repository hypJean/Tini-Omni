...
@dataclass
class VocabConfig:
    text_vocabsize: int = 262140
    text_specialtokens: int = 4
...
class DataConfig:
    ...
    train_data_ratio: float = field(default=1.0, metadata={
        "help": "Ratio of training data to use (0.0-1.0). For example, 0.33 means use 1/3 of the data."
    })
...
class DecodeConfig:
    ...
    # When generation hits max_new_tokens, optionally force-decode the first N audio tokens
    # If 0 (default), the generator keeps previous behavior (treat as failure and return None)
    force_audio_tokens: int = 0
...
class LogConfig:
    ...
    wandb_dir: str = "/data/zhouyuran/run/exp/wandb_log"
    wandb_entity_name: str = "project_name"
    wandb_project_name: str = "project_name"
    wandb_exp_name: str = "exp_name"
    log_file: str = "/data/zhouyuran/run/exp/log/test.log"
    log_interval: int = 10

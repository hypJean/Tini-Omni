...
from utils.snac_utils import simple_shift
...
def model_factory(train_config, model_config, **kwargs):
    ...
    if ckpt_path is not None:
        ...
        if missing_keys:
            logger.warning(f"Missing keys when loading checkpoint (showing first 20): {missing_keys[:20]}")
        if unexpected_keys:
            logger.warning(f"Unexpected keys in checkpoint (showing first 20): {unexpected_keys[:20]}")
        logger.info(f"Successfully loaded checkpoint with {len(ckpt_dict)} parameter groups")
...
class slam_model_s2s(slam_model):
    ...
    def forward(self,
        ...
        model_outputs = self.llm(inputs_embeds=inputs_embeds, attention_mask=attention_mask, labels=text_labels, output_hidden_states=True)    # here we use the text token layer as the target label
        ...
        total_audio_vocab_size = self.model_config.vocab_config.total_audio_vocabsize
        xt = x_ori[..., :text_vocab_size]
        xa = []

        if self.group_decode_adapter is not None:
            # Use hidden_states (llm_dim) instead of logits (total_audio_vocabsize)
            hidden_states = model_outputs.hidden_states[-1]  # Get the last layer hidden states
            x_audio = self.group_decode_adapter(hidden_states)
    ...
    def generate(self,
        ...
        for step in tqdm(range(max_new_tokens), desc="Generating"):
            if current_input_text is not None:
                ...
                # First iteration: use inputs_embeds from forward (contains audio features)
                pass
            ...
            outputs = self.llm(
                ...
                past_key_values=past_key_values,
                use_cache=True,
                output_hidden_states=True,  # 需要 hidden_states 用于 group_decode_adapter
            )
            ...
            if self.group_decode_adapter is not None:
                # 使用 hidden_states 而不是 logits
                hidden_states = outputs.hidden_states[-1][0]  # Get the last layer hidden states, batch size is 1
                xa_logits = self.group_decode_adapter(hidden_states)

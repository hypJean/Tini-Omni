...
def setup_codec(train_config, model_config, **kwargs):
    if model_config.codec_decoder_type == "CosyVoice":
        ...
        # 兼容较新版本的 huggingface_hub：为 diffusers 提供 cached_download 别名
        try:
            ...
            if not hasattr(_hfh, "cached_download"):
                def cached_download(*args, **kwargs):
                    # 直接转调到 hf_hub_download，行为基本等价
                    return _hf_hub_download(*args, **kwargs)

                _hfh.cached_download = cached_download
                logger.warning(
                    "huggingface_hub.cached_download not found; "
                    "patched alias to hf_hub_download for CosyVoice/diffusers compatibility."
                )
        except Exception as e:
            logger.warning(f"Failed to patch huggingface_hub.cached_download: {e}")
        ...
        raise NotImplementedError(f"Unsupported codec_decoder_type: {model_config.codec_decoder_type}. Only CosyVoice is supported.")
...
def audio_decode_cosyvoice(audio_tokens, model_config, codec_decoder, tone_dir, audio_prompt_path=None, code_layer=1, num_latency_tokens=1, speed=1.0, replace_token=4095):
    ...
    # Truncate audio tokens at the EOA token (robust if EOA not present)
    positions = torch.nonzero(audio_tokens[0] == eoa, as_tuple=False).squeeze(-1)
    if positions.numel() == 0:
        # No explicit EOA found in the token sequence — append a safe terminator token
        # Use `replace_token` (default 4095) which is within the valid codec token range
        # to avoid out-of-range embedding indices that trigger device-side asserts.
        safe_term = torch.tensor([[replace_token]], device=audio_tokens.device, dtype=audio_tokens.dtype)
        audio_tokens = torch.cat([audio_tokens, safe_term], dim=-1)
    else:
        end_index = int(positions[0].item())

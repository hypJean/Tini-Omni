...
class SpeechDatasetJsonl(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.train_data_ratio = dataset_config.get("train_data_ratio", 1.0)
        assert self.input_type in ["raw", "mel"], "input_type must be one of [raw, mel]" 
        assert self.manifest_format in ["parquet", "jsonl"], "manifest_format must be one of [parquet, jsonl]"
        assert 0.0 < self.train_data_ratio <= 1.0, "train_data_ratio must be in (0.0, 1.0]"
        ...
        # code type config
        self.code_type = dataset_config.get("code_type", "CosyVoice")
        if self.code_type != "CosyVoice":
            raise ValueError("code_type must be CosyVoice")
        ...
        if self.manifest_format == "parquet":
            ...
            # If load_from_cache_file is True we treat train_data_path as a HF dataset id
            if dataset_config.load_from_cache_file:
                ds = load_dataset(dataset_config.train_data_path)       # load_from huggingface datasets
            else:
                # If train_data_path is a local directory containing parquet shard files
                # (e.g., HF cache downloads without extensions), try to load those
                # parquet files directly. This allows training on already-downloaded
                # shards without first calling save_to_disk (avoids extra disk usage).
                import os, glob
                path = dataset_config.train_data_path
                if os.path.isdir(path):
                    # collect candidate files that are not the small .json/.lock metadata
                    candidates = [os.path.join(path, f) for f in os.listdir(path)
                                  if os.path.isfile(os.path.join(path, f)) and
                                  (not f.endswith('.json') and not f.endswith('.jsonl') and not f.endswith('.lock'))]
                    if len(candidates) > 0:
                        # filter only readable parquet files to avoid corrupted/incomplete shards
                        good = []
                        try:
                        ...
                        if len(good) > 0:
                            ds = load_dataset('parquet', data_files={'train': good})
                        else:
                            # fallback to load_from_disk if no good parquet found
                            ds = load_from_disk(path)
                    else:
                        ds = load_from_disk(path)
                else:
                    ds = load_from_disk(path)   # load_from local disk
            train_val_split = ds['train'].train_test_split(test_size=self.split_size, seed=self.seed)
            if split == "train":
                self.data_list = train_val_split['train']
                # Limit training data to specified ratio (use first N samples, same every time)
                if self.train_data_ratio < 1.0:
                    total_size = len(self.data_list)
                    limit_size = int(total_size * self.train_data_ratio)
                    self.data_list = self.data_list.select(range(limit_size))
            else:
            ...
            if split == "train":
                ...
                if self.train_data_ratio < 1.0:
                    total_size = len(self.data_list)
                    limit_size = int(total_size * self.train_data_ratio)
                    self.data_list = self.data_list[:limit_size]
            else:
    ...
    def extract_audio_feature(self, audio_path):
        ...
        if self.manifest_format == "parquet" and isinstance(audio_path, dict):
            ...
            if self.code_type == "CosyVoice":
                ...
                raise ValueError(f"Unsupported code_type: {self.code_type}. Only CosyVoice is supported.")

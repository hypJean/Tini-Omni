...
def generate_from_wav(wav_path, model, codec_decoder, dataset_config, decode_config, logger, device, model_config, tone_dir, audio_prompt_path=None, output_text_only=False, layer_shift=layershift):
	...
	logger.info(f"[DEBUG] Processing: {wav_path}, mel_shape: {audio_mel.shape}, length: {audio_length}, mean: {audio_mel.mean().item():.6f}, std: {audio_mel.std().item():.6f}")
	
	# Debug: Use whisper to transcribe the audio to check if model understands the audio content
	try:
		whisper_model_path = model_config.encoder_path
		whisper_model = whisper.load_model(whisper_model_path)
		whisper_model = whisper_model.to(device)  # Move to the same device as audio_mel
		# audio_mel shape: [seq_len, n_mels], need to permute to [n_mels, seq_len] for whisper
		audio_mel_for_decode = audio_mel.permute(1, 0).to(device)  # [seq_len, n_mels] -> [n_mels, seq_len]
		# Specify language to avoid "This model doesn't have language tokens" error
		# Check if model path contains language indicator (e.g., "small.en.pt" means English)
		if ".en." in whisper_model_path.lower():
			options = whisper.DecodingOptions(language="en", without_timestamps=True)
		else:
			# Try to detect language or use default
			options = whisper.DecodingOptions(without_timestamps=True)
		result = whisper.decode(whisper_model, audio_mel_for_decode, options)
		transcribed_text = result.text
		logger.info(f"[DEBUG] Whisper ASR transcription: {transcribed_text}")
	except Exception as e:
		logger.warning(f"[DEBUG] Failed to transcribe audio with whisper: {e}")
	...
	logger.info(f"[DEBUG] Modality mask - sum: {modality_mask.sum().item()}, audio_length: {audio_length.item()}, padding_left: {padding_left}")
	...
	logger.info(f"[DEBUG] Generated text (first 150 chars): {output_text[:150]}")
	
	if decode_config.decode_text_only or output_text_only:
		return None, output_text

	if audio_outputs[0].shape[0] == decode_config.max_new_tokens:
		# If the model ran until max_new_tokens, it may be still generating.
		# Optionally allow force-truncation to decode the first N audio tokens instead of skipping.
		force_n = int(getattr(decode_config, "force_audio_tokens", 0))
		if force_n > 0:
			logger.warning(f"Audio token reached max_new_tokens; force-truncating to first {force_n} tokens.")
			audio_outputs = [audio_outputs[layer][:force_n] for layer in range(code_layer)]
		else:
	...
	if code_type == "CosyVoice":
		...
		raise NotImplementedError(f"Unsupported code_type: {code_type}. Only CosyVoice is supported.")
...
def generate_from_text(text_input, model, codec_decoder, dataset_config, decode_config, logger, device, model_config, tone_dir, audio_prompt_path=None, output_text_only=False, layer_shift=layershift):
	...
	if audio_outputs[0].shape[0] == decode_config.max_new_tokens:
		# If the model ran until max_new_tokens, optionally force-truncate first N tokens.
		force_n = int(getattr(decode_config, "force_audio_tokens", 0))
		if force_n > 0:
			logger.warning(f"Audio token reached max_new_tokens; force-truncating to first {force_n} tokens.")
			audio_outputs = [audio_outputs[layer][:force_n] for layer in range(code_layer)]
...
def main(kwargs: DictConfig):
	...
	if decode_config.input_text:
		...
		logger.info("============== Ready for s2s Online Inference ==============")
		while True:
